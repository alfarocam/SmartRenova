...
public class RegistroService {
    ...
    @Value("${servidor.dominio:localhost}")
    private String servidor;

    public Model activar(Model model, String username, String clave) {
        Usuario usuario = usuarioService.findByUsernameAndPassword(username, clave);
        if (usuario != null) {
    ...
    public Model recordarUsuario(Model model, Usuario usuario) throws MessagingException {
        String mensaje;
        Usuario usuario2 = usuarioService.findByUsernameOrCorreo(usuario.getUsername(), usuario.getCorreo());
        if (usuario2 != null) {
            String clave = demeClave();
